...
public class ApkEditorActivity extends Activity {
    ...
    private static final String PREFS_SIGNING =
            "apk_signing_config";

    private static final String PREF_KEYSTORE_URI =
            "keystore_uri";

    private static final String PREF_ALIAS =
            "alias";

    private static final String PREF_STORE_PASSWORD =
            "store_password";
    ...
    private void createInterface() {
        ...
        editorContainer.setOrientation(
                LinearLayout.VERTICAL
        );
    ...
    private void showDexList(
        ...
        if (names == null ||
            ...
            statusText.setText(
                    "❌ No se encontraron DEX."
            );

            editorContainer.addView(
                    text(
                            "No se encontraron classes*.dex.",
                            14
                    ),
                    params(-1, -2)
            );
...
private void showClassList(
    ...
    classesContainer.setOrientation(
            LinearLayout.VERTICAL
    );
...
private int getClassPriority(String className) {
    ...
    private void showClassDetails(
        ...
        try {
            ...
            for (Method method :
                ...
                methodButton.setText(
                        buildMethodDisplaySignature(
                                selectedMethod
                        )
                );
    ...
    private String buildMethodDisplaySignature(
            Method method) {
    ...
    private String disassembleMethodWithBaksmali(
            Method method,
            String className)
            throws Exception {
        ...
        if (method.getImplementation()
            ...
            return buildEmptyMethodWithBaksmali(
                    method
            );
        }
    ...
    private ClassDef findClassForMethod(
            File dexFile,
            String className)
            throws Exception {
    ...
    private String buildEmptyMethodWithBaksmali(
            Method method) {
    ...
    private void buildModifiedApk(
            String modifiedSmali,
            Method editedMethod) {
        ...
        new Thread(() -> {
            ...
            try {
                ...
                if (!buildSmaliDir.mkdirs() &&
                        !buildSmaliDir.exists()) {
                ...
                SmaliManager smaliManager =
                        new SmaliManager(35);

                smaliManager.disassemble(
                        buildOriginalDex,
                        buildSmaliDir
                );
                ...
                updateBuildStatus(
                        "4/6 → Aplicando modificación..."
                );

                String originalClassSmali =
                        readTextFile(
                                classSmali
                        );

                String newClassSmali =
                ...
                updateBuildStatus(
                        "5/6 → Recompilando DEX..."
                );

                smaliManager.assemble(
                        buildSmaliDir,
                        buildNewDex
                );
    ...
    private File findSmaliClassFile(
            File smaliDirectory,
            String className) {
    ...
    private File findFileRecursive(
            File directory,
            String expectedPath) {
    ...
    private String replaceMethodInClass(
            String classSmali,
            String newMethodSmali,
            Method method)
            throws Exception {

        if (classSmali == null ||
                classSmali.trim().isEmpty()) {

            throw new Exception(
                    "La clase Smali está vacía."
            );
        }

        if (newMethodSmali == null ||
                newMethodSmali.trim().isEmpty()) {

            throw new Exception(
                    "El método modificado está vacío."
            );
        }
    ...
    private boolean methodDeclarationMatches(
            String declaration,
            String prototype,
            String methodName) {
    ...
    private String normalizeMethodText(
            String value) {
        ...
        if (!result.startsWith(
            ...
            throw new IllegalArgumentException(
                    "El contenido debe comenzar con .method"
            );
        }
        ...
        if (!result.contains(
            ...
            throw new IllegalArgumentException(
                    "El contenido no contiene .end method"
            );
        }
    ...
    private void extractDexFromApk(
            File apk,
            String dexName,
            File outputDex)
            throws Exception {
    ...
    private void saveSigningConfig(
            Uri keystoreUri,
            String alias,
            String storePassword,
            String keyPassword) {
    ...
    private void showBuildFinishedDialog(
            File unsignedApk) {
        ...
        new AlertDialog.Builder(this)
                ...
                .setMessage(
                        "Todavía no hay una firma configurada.\n\n" +
    ...
    private void signAutomatically() {
        ...
        if (!hasAutomaticSigningConfig()) {
            ...
            Toast.makeText(
                    this,
                    "❌ No hay configuración de firma guardada.",
                    Toast.LENGTH_LONG
            ).show();
        ...
        if (keystoreUri == null) {
            ...
            Toast.makeText(
                    this,
                    "❌ No se encontró el keystore guardado.",
                    Toast.LENGTH_LONG
            ).show();
        ...
        if (pendingKeyPassword == null ||
                pendingKeyPassword.isEmpty()) {
    ...
    private void saveUnsignedApk() {
        ...
        if (buildUnsignedApk == null ||
                !buildUnsignedApk.exists()) {
    ...
    private void showKeystoreDialog(
        ...
        if (savedStorePassword != null &&
                !savedStorePassword.isEmpty()) {
        ...
        if (savedKeyPassword != null &&
                !savedKeyPassword.isEmpty()) {
        ...
        new AlertDialog.Builder(this)
                ...
                .setMessage(
                        "Introduce los datos de tu firma.\n\n" +
                        "Estos datos se guardarán para que " +
                        "las próximas compilaciones se firmen " +
                        "automáticamente."
                )
                .setView(layout)
                .setPositiveButton(
    ...
    private void signBuiltApk(
        ...
        if (pendingAlias == null ||
                pendingAlias.trim().isEmpty()) {

            Toast.makeText(
                    this,
                    "❌ Alias de firma vacío.",
                    Toast.LENGTH_LONG
            ).show();
        ...
        if (pendingKeystorePassword == null ||
                pendingKeystorePassword.isEmpty()) {
        ...
        new Thread(() -> {
            ...
            try {
                ...
                if (!keystoreFile.exists() ||
                    ...
                    throw new Exception(
                            "El keystore copiado está vacío."
                    );
                }
                ...
                if (!buildSignedApk.exists() ||
                        buildSignedApk.length() == 0) {

                    throw new IOException(
                            "El APK firmado no fue generado."
                    );
                }
    ...
    private void showSignedApkDialog() {
        ...
        if (buildSignedApk == null ||
                !buildSignedApk.exists()) {
        ...
        new AlertDialog.Builder(this)
                ...
                .setMessage(
                        "El APK fue firmado correctamente.\n\n" +
                        "Tamaño: " +
                        buildSignedApk.length() +
                        " bytes.\n\n" +
    ...
    protected void onActivityResult(
        ...
        if (requestCode ==
            ...
            if (source == null ||
                ...
                Toast.makeText(
                        this,
                        "❌ No se encontró el APK que se debe guardar.",
                        Toast.LENGTH_LONG
                ).show();
...
private void showSearchResults(
    ...
    resultCounter.setTextIsSelectable(
            true
    );
    ...
    final LinearLayout resultsContainer =
            new LinearLayout(this);
